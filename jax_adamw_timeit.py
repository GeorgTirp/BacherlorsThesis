...
def init_optimizer_state(workload: spec.Workload,
  ...
  if GRAPH["make_graph"]:
    ...
    plt.grid(True)
    figname = '/experiment_runs/wmt_learning_rate_plot' + str(GRAPH["counter"])+ '.pdf'
    plt.savefig(figname)
    plt.clf()
...
def model_reinit_schedule(
  ...
  step_hint = workload.step_hint * 2
...
def update_params(workload: spec.Workload,
  ...
  new_params, new_optimizer_state = model_reinit_schedule(
    global_step,
    workload,
    SCHEDULE_PARAMS,
    rng,
  ...
  combined_vars = globals().copy()
  combined_vars.update(locals())
  if global_step % 100 == 0:
    combined_vars = globals().copy()
    combined_vars.update(locals())
    execution_time_func = timeit.timeit('model_reinit_schedule(global_step,workload,SCHEDULE_PARAMS,rng,hyperparameters,new_model_state,new_optimizer_state,new_params)', globals=combined_vars, number=1)
    print(f"Execution time of reinit func: {execution_time_func} seconds")
    code_snippet = """
...
"""
    ...
    execution_time = timeit.timeit(code_snippet, globals=combined_vars, number=1)
    print(f"Execution time of the code snippet: {execution_time} seconds")
...
"""
        ...
        execution_time2 = timeit.timeit(code_snippet2, globals=combined_vars, number=1)
        print(f"Execution time of the code snippet 2: {execution_time2} seconds") 
