...
def init_optimizer_state(workload: spec.Workload,
  ...
  if GRAPH["make_graph"]:
    ...
    plt.grid(True)
    figname = '/experiment_runs/early_stop2_learning_rate_plot' + str(GRAPH["counter"])+ '.pdf'
    plt.savefig(figname)
    plt.clf()
...
def model_reinit_schedule(
  ...
  is_stopping = is_stopping_early(schedule_params, epsilon=0.01, epsilon2= 0.01,recovery_time=9)
  ...
  if global_step % 100 == 0 and workload.metrics_logger is not None:
    if SCHEDULE_PARAMS["stop_metric"]["metrics"] != [] and eval_results != []:
      ...
      if current_vloss != last_vloss:
        ...
        smoothed_vloss = 0.6 * current_vloss + 0.4 * last_smoothed_vloss
